...
void zmq_init(void * arg, zmq::context_t* context,Sockets* socks,  int* forward, int* back) {
    ...
    if(param->out_grouping.size()>0) {
        if(param->out_grouping.back() == "shuffle") {
            ...
            socks->sender = shuffle_sender_conn(param, *context, senderIP[0], senderPort[0]);
        } else if(param->out_grouping.back() == "key") {
            std::cout << "Setting up key sender " << std::endl;
            socks->sender = key_sender_conn(param, *context, senderIP, senderPort);
        }
    }
    if(param->in_grouping.size()>0) {
        if(param->in_grouping.back() == "shuffle") {
            std::cout << "Setting up shuffle receiver " << std::endl;
            socks->receiver = shuffle_receiver_conn(param, *context, receiverIP, receiverPort);
        } else if(param->in_grouping.back() == "key") {
            std::cout << "Setting up key receiver " << std::endl;
            socks->receiver = key_receiver_conn(param, *context, receiverIP[0], receiverPort[0]);
        }
    }
